...
// hero equipment, so every machine reads as built from consistent parts
// rather than one-off shapes.
...
export function FanBlades({ radius = 0.42, color = COLORS.metalLight, spinRef }) {
  ...
  return (
    <group ref={groupRef}>
      ...
      <mesh>
        <cylinderGeometry args={[radius * 0.18, radius * 0.18, 0.08, 12]} rotation={[Math.PI / 2, 0, 0]} />
...
export function Gauge({ radius = 0.09, glowRef, color = COLORS.energyCyan }) {
  ...
  return (
    <group>
      <mesh>
        <cylinderGeometry args={[radius, radius, 0.02, 16]} rotation={[Math.PI / 2, 0, 0]} />
...
// Straight pipe run between two points, with elbow caps.
...
// An L-shaped pipe run through an intermediate elbow point (for tidy 90°
// routing between equipment at different heights, avoids pipes cutting
