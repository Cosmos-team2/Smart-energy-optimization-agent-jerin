...
export async function POST(
  ...
  try {
    ...
    console.error(`[route:POST /api/recommendations/${id}/reject] Backend returned ${backendRes.status}: ${errorBody}`);
    return NextResponse.json(
      { error: `Backend error: ${backendRes.status}`, detail: errorBody },
      { status: backendRes.status }
    );
  } catch (err) {
    console.warn(`[route:POST /api/recommendations/${id}/reject] Backend offline, falling back to canonical fixture:`, err);
  }
